...
class SubCategoryController extends Controller {
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post()) && Yii::$app->SetValues->Attributes($model)) {
            ...
            if ($model->validate() && $model->save()) {
                if (!empty($image)) {
                    $path = Yii::$app->basePath . '/../uploads/product-subcategory/' . $model->id . '/';
                    $size = [['width' => 100, 'height' => 100, 'name' => 'small'], ['width' => 175, 'height' => 120, 'name' => 'image'],];
                    Yii::$app->UploadFile->UploadFile($model, $image, $path, $size);
                }
                Yii::$app->session->setFlash('success', "New subcategory added successfully");
                $model = new SubCategory();
            }
    ...
    public function actionDelete($id) {
        $products = \common\models\Products::find()->where(['sub_category' => $id])->all();
        if (empty($products)) {
            if ($this->findModel($id)->delete()) {
                $path = Yii::$app->basePath . '/../uploads/product-subcategory/' . $id;
                $this->deleteDir($path);
            }
            Yii::$app->session->setFlash('success', "subcategory removed successfully");
        } else {
            Yii::$app->session->setFlash('success', "Can't remove");
        }
